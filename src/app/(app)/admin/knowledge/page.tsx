...
export default function AdminKnowledgePage() {
  ...
  const handleDelete = useCallback(
    async (pageId: string) => {
      try {
        const res = await fetch(`/api/admin/knowledge/${pageId}`, { method: 'DELETE' })
        if (res.ok) {
    ...
    <div className="space-y-6">
      ...
      {flatPages.length === 0 ? (
        <div className="rounded-xl border border-border bg-surface p-12 text-center">
          <p className="text-foreground-muted">No knowledge pages yet. Create your first one.</p>
        </div>
      ) : (
      ...
      {showNewModal && (
        <NewPageModal
          ...
          onCreated={(newPage) => {
            ...
            router.push(`/admin/knowledge/${newPage.id}`)
          }}
        />
      )}
    </div>
...
function NewPageModal({
  ...
  const handleSubmit = useCallback(
    ...
    [title, slug, parentId, visibility, customGroup, onCreated]
  )

  // Build flat parent options
  const parentOptions = flattenWithDepthStatic(pages)

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-lg rounded-xl border border-border bg-surface p-6 shadow-lg">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-foreground">New Knowledge Page</h2>
          <button
            onClick={onClose}
...
/** Static version for the modal (no React state needed) */
function flattenWithDepthStatic(pages: AdminPage[]): FlatPage[] {
  const childrenMap = new Map<string | null, AdminPage[]>()
  for (const page of pages) {
    const key = page.parent_id ?? null
    if (!childrenMap.has(key)) childrenMap.set(key, [])
