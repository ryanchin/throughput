...
/**
 * Training course results page.
 * Displays the completion scorecard for an enrolled user.
 */
export default async function TrainingResultsPage({
  ...
  const data = await getResultsData(courseSlug, 'training')
  if (!data) notFound()
...
async function getResultsData(courseSlug: string, zone: string) {
  ...
  // Fetch lessons, quizzes, attempts, and questions for breakdown
