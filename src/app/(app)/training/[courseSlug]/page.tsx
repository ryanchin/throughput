...
export default async function TrainingCourseOverviewPage({
  params,
}: {
  params: Promise<{ courseSlug: string }>
}) {
  const { courseSlug } = await params
  const data = await getCourseData(courseSlug, 'training')

  if (!data) {
    // Could be unauthenticated, wrong zone, or not found
  ...
  // Build lessonProgress and quizInfo arrays for LessonNav
  ...
  const quizInfo = lessons
    ...
    .map((l) => ({
      lessonId: l.id,
      passed: false, // Overview page doesn't need individual quiz pass data
    }))
